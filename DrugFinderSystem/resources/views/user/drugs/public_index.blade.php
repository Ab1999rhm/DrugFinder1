...
<!-- Login Please Modal -->
<div class="modal fade" id="loginPleaseModal" tabindex="-1" role="dialog" aria-labelledby="loginPleaseModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="loginPleaseModalLabel">Login Required</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body text-center">
                <p style="font-size:1.15rem;color:red;font-weight:600;">Your drug is found in. if you login you can access all of drugs without restriction!, login please</p>
                <li class="nav-item">
...
<style>
    p {
        background-image: url("{{ asset('images/ph4.jpg') }}");
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // Intercept Buy, Wishlist, and Filter buttons for guests and show modal instead
        $(document).on('click', '.buy-btn, .wishlist-btn, .filter-btn', function(e) {
            // Only block if not already inside the login modal
            if (!$(this).closest('.modal').length) {
                e.preventDefault();
                $('#loginPleaseModal').modal('show');
            }
        });

        // Sidebar Filter Submission (AJAX) - will be blocked by above for guests
        $('#sidebar-filter-form').on('submit', function(e) {
            // If modal is shown, do nothing (handled by above)
            if ($('#loginPleaseModal').hasClass('show')) {
                e.preventDefault();
                return;
            }
            $('#loader').show();
        ...
        $(document).on('click', '.details-btn', function() {
            ...
            $('#drug-detail-content').html('<div class="text-center py-5"><i class="fas fa-spinner fa-spin fa-2x"></i></div>');
            $('#drugDetailModal').modal('show');
            $.get('/user/drugs/' + drugId + '/Buy', function(html) {
                $('#drug-detail-content').html(html);
            });
        });

        // Order Form Handler (if used) - will be blocked by above for guests
